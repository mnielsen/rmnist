...
#td = RMNIST(10)
#img = data_transforms["train"](td.__getitem__(5)[0])
...
image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x),
                                          data_transforms[x])
                  for x in ['train', 'val']}
dataloders_old = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=4,
                                             shuffle=True, num_workers=4)
                  for x in ['train', 'val']}
dataloders = {"train": torch.utils.data.DataLoader(
    RMNIST(10, transform=data_transforms["train"]), batch_size=4, shuffle=True, num_workers=4),
              "val": torch.utils.data.DataLoader(
    RMNIST(10, transform=data_transforms["val"], train=False), batch_size=4, shuffle=True, num_workers=4)}
dataset_sizes = {"train": 100, "val": 10000}
